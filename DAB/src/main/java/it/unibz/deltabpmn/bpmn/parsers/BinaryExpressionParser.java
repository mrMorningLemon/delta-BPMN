...
public class BinaryExpressionParser {
    ...
    public static BinaryCondition parse(String expression, DataSchema dataSchema) {
        ...
        if (expression.contains("=")) {
            ...
            condition = BinaryConditionProvider.equality(TermProcessor.processTerm(operands[0].trim(), dataSchema), TermProcessor.processTerm(operands[1].trim(), dataSchema));
        }
        if (expression.contains("!=")) {
            String[] operands = expression.split("!=");
            condition = BinaryConditionProvider.inequality(TermProcessor.processTerm(operands[0].trim(), dataSchema), TermProcessor.processTerm(operands[1].trim(), dataSchema));
        }
        if (expression.contains(">")) {
            String[] operands = expression.split(">");
            condition = BinaryConditionProvider.greaterThan(TermProcessor.processTerm(operands[0].trim(), dataSchema), TermProcessor.processTerm(operands[1].trim(), dataSchema));
        }
        if (expression.contains("<")) {
            String[] operands = expression.split("<");
            condition = BinaryConditionProvider.inequality(TermProcessor.processTerm(operands[0].trim(), dataSchema), TermProcessor.processTerm(operands[1].trim(), dataSchema));
